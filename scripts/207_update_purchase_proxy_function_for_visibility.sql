-- =====================================================
-- FIX: UPDATE purchase_proxy_plan FUNCTION FOR VISIBILITY LOGIC
-- This script modifies the function to:
-- 1. Select only 'private' and unassigned proxies for purchase.
-- 2. Assign the purchased proxy to the user by updating its user_id in the proxies table.
-- =====================================================

-- Drop the function to ensure any old signatures are removed
DROP FUNCTION IF EXISTS purchase_proxy_plan(UUID, UUID);

-- Recreate the complete purchase_proxy_plan function with updated logic
...
BEGIN
    ...
    -- Tìm proxy khả dụng (chỉ chọn proxy PRIVATE và chưa được gán cho người dùng nào)
    SELECT p.id INTO v_selected_proxy_id
    FROM proxies p
    WHERE p.is_active = TRUE
      AND p.type = v_plan_type
      AND p.current_users < p.max_users
      AND p.visibility = 'private' -- Chỉ chọn proxy private để bán
      AND p.user_id IS NULL      -- Chỉ chọn proxy chưa được gán cho người dùng nào
    ...
    BEGIN
        ...
        -- Gán proxy cho người dùng và đảm bảo visibility là 'private'
        UPDATE proxies
