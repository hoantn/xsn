-- =====================================================
-- SCRIPT TRIỂN KHAI HỆ THỐNG HOÀN CHỈNH (PHIÊN BẢN V4)
-- Đồng bộ hóa tên cột 'account_name' cho chủ tài khoản ngân hàng
...
ON CONFLICT (name) DO NOTHING; -- Sử dụng name làm unique constraint cho sample data
...
ON CONFLICT (url) DO NOTHING; -- Giả định URL là duy nhất cho các proxy mẫu
