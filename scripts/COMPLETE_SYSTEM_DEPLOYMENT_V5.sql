-- =====================================================
-- SCRIPT TRIỂN KHAI HỆ THỐNG HOÀN CHỈNH (PHIÊN BẢN V5)
-- Khắc phục lỗi ON CONFLICT: Thêm ràng buộc UNIQUE cho các cột cần thiết.
-- Đảm bảo tính idempotency và không mất dữ liệu cũ.
-- =====================================================
...
ON CONFLICT (name) DO NOTHING; -- Bây giờ đã có ràng buộc UNIQUE trên cột 'name'
...
ON CONFLICT (username) DO UPDATE SET
    ...
    is_active = EXCLUDED.is_active; -- Không update balance để tránh reset nếu đã có
...
ON CONFLICT (account_number) DO UPDATE SET
    ...
    account_name = EXCLUDED.account_name, -- Sử dụng account_name
    qr_template = EXCLUDED.qr_template,
    is_active = EXCLUDED.is_active;

-- 18. Tạo một số proxy mẫu để test (ON CONFLICT DO NOTHING)
INSERT INTO proxies (url, server, port, type, description, visibility, is_active, max_users, source, secret)
VALUES
    ('tg://proxy?server=proxy1.example.com&port=443&secret=abcd1234', 'proxy1.example.com', 443, 'mtproto', 'Proxy MTProto test 1', 'public', true, 1, 'Manual', 'abcd1234'),
    ('tg://proxy?server=proxy2.example.com&port=443&secret=efgh5678', 'proxy2.example.com', 443, 'mtproto', 'Proxy MTProto test 2', 'public', true, 1, 'Manual', 'efgh5678'),
    ('tg://proxy?server=proxy3.example.com&port=443&secret=ijkl9012', 'proxy3.example.com', 443, 'mtproto', 'Proxy MTProto test 3', 'public', true, 1, 'Manual', 'ijkl9012'),
    ('tg://proxy?server=proxy4.example.com&port=443&secret=mnop3456', 'proxy4.example.com', 443, 'mtproto', 'Proxy MTProto test 4', 'public', true, 1, 'Manual', 'mnop3456'),
    ('tg://proxy?server=proxy5.example.com&port=443&secret=qrst7890', 'proxy5.example.com', 443, 'mtproto', 'Proxy MTProto test 5', 'public', true, 1, 'Manual', 'qrst7890')
ON CONFLICT (url) DO NOTHING; -- Bây giờ đã có ràng buộc UNIQUE trên cột 'url'
