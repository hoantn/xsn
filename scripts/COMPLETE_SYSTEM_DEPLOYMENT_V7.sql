-- =====================================================
-- SCRIPT TRIỂN KHAI HỆ THỐNG HOÀN CHỈNH (PHIÊN BẢN V7)
-- Khắc phục lỗi: Thêm 'proxy_purchase' và 'refund' vào CHECK constraint của transactions.type.
-- Cập nhật các ràng buộc UNIQUE và cột 'secret'.
...
CREATE TABLE IF NOT EXISTS proxies (
  ...
  created_at TIMESTAMPTZ DEFAULT NOW(),
  updated_at TIMESTanz DEFAULT NOW()
);
...
CREATE TABLE IF NOT EXISTS transactions (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id UUID NOT NULL REFERENCES users(id) ON DELETE CASCADE,
  type VARCHAR(50) NOT NULL CHECK (type IN ('deposit', 'proxy_purchase', 'admin_adjustment', 'refund', 'initial_balance')), -- ĐÃ CẬP NHẬT CHECK CONSTRAINT
