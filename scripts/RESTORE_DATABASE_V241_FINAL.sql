-- =====================================================
-- KHÔI PHỤC DATABASE CHO PHIÊN BẢN V241 (FINAL)
-- Script này sẽ tái tạo lại database theo đúng cấu trúc
-- mà code v241 đang sử dụng - Xử lý tất cả conflicts
-- =====================================================
...
-- 11. Thêm admin accounts mặc định (chỉ thêm nếu chưa có - sử dụng ON CONFLICT)
INSERT INTO users (username, password_hash, role, full_name, balance) VALUES 
  ('admin', '$2a$10$rQJ5qJZvQJ5qJZvQJ5qJZuO', 'admin', 'Administrator', 0),
  ('superadmin', '$2a$10$rQJ5qJZvQJ5qJZvQJ5qJZuO', 'super_admin', 'Super Administrator', 0)
ON CONFLICT (username) DO NOTHING;

-- 12. Thêm dữ liệu mẫu cho proxy_plans (chỉ thêm nếu bảng trống)
INSERT INTO proxy_plans (name, description, price, duration_days, max_connections) 
SELECT 'Gói Cơ Bản', 'Proxy MTProto cơ bản cho 1 thiết bị', 50000, 30, 1
WHERE NOT EXISTS (SELECT 1 FROM proxy_plans WHERE name = 'Gói Cơ Bản');

INSERT INTO proxy_plans (name, description, price, duration_days, max_connections) 
SELECT 'Gói Tiêu Chuẩn', 'Proxy MTProto cho 3 thiết bị', 120000, 30, 3
WHERE NOT EXISTS (SELECT 1 FROM proxy_plans WHERE name = 'Gói Tiêu Chuẩn');

INSERT INTO proxy_plans (name, description, price, duration_days, max_connections) 
SELECT 'Gói Premium', 'Proxy MTProto không giới hạn thiết bị', 200000, 30, 999
