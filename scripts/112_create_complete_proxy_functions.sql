...
-- Hàm mua proxy HOÀN CHỈNH với tất cả các cột
...
BEGIN
    ...
    IF NOT FOUND THEN
        RETURN QUERY SELECT FALSE, 'Gói proxy không tồn tại hoặc đã bị vô hiệu hóa', NULL, NULL;
        RETURN;
    END IF;
    ...
    IF v_selected_proxy_id IS NULL THEN
        RETURN QUERY SELECT FALSE, 'Không có proxy ' || v_plan_type || ' khả dụng cho gói này', NULL, NULL;
        RETURN;
    END IF;
    ...
    BEGIN
        ...
        -- Tạo đơn hàng proxy với TẤT CẢ các cột cần thiết
        ...
        VALUES (
            ...
            p_plan_id, 
            v_selected_proxy_id, 
            v_plan_price,           -- unit_price
            v_plan_price,           -- total_amount  
            v_plan_duration_days,   -- duration_days
            'active',               -- status
        ...
        -- Cập nhật current_users (trigger sẽ tự động xử lý, nhưng để chắc chắn)
        UPDATE proxies
...
RAISE NOTICE 'Đã tạo hoàn chỉnh tất cả hàm và trigger cho hệ thống proxy';
